...
if (typeof API === 'undefined') {
   ...
   async fetchProgress(promptId) {
        try {
            // Point to Laravel instead of ComfyUI directly
            const response = await fetch(`/api/comfyui/progress?prompt_id=${promptId}`);
    ...
    async checkComfyUI() {
        try {
            // Use the health check route we built in Laravel
            ...
            console.log('ComfyUI check:', e.message);
            Utils.updateStatus("⚠️ ComfyUI unreachable via Laravel Proxy", "warning");
            return false;
        }
    },
    ...
    async getObjectInfo() {
        try {
            const response = await fetch(`http://${Config.COMFYUI_HOST}:${Config.COMFYUI_PORT}/object_info`);
    ...
    async clearQueue() {
        try {
            const response = await fetch(`http://${Config.COMFYUI_HOST}:${Config.COMFYUI_PORT}/queue`, {
                method: 'DELETE'
            });
