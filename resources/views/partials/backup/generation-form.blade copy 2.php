    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (healthBtn) {
            healthBtn.onclick = async () => {
                ...
                statusDiv.innerHTML = '🔄 Checking ComfyUI connection via Laravel Proxy...';
                statusDiv.style.color = '#666';
                
                try {
                    // Call Laravel Proxy instead of port 8188 directly
                    const response = await fetch('/api/comfyui/health');
                    const data = await response.json();
                    
                    if (data && data.success) {
                        statusDiv.innerHTML = `✅ ComfyUI is Online<br><small>GPU: ${data.gpu_name}</small>`;
                        statusDiv.style.color = '#155724';
                        
                        // Update the VRAM meter if we have the data
                        if (vramPanel && data.vram_total) {
                            ...
                            document.getElementById('gpuName').innerText = data.gpu_name;
                        }
                    } else {
                        throw new Error(data.error || 'Server error');
                    }
                } catch (error) {
                    statusDiv.innerHTML = `❌ Connection Error: ${error.message}<br>
                    <small>Check if ComfyUI is running on port 8188 inside your WSL/Linux terminal.</small>`;
