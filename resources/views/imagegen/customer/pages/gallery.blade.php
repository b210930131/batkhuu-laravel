...
@section('page_subtitle', 'Manage your generated images and folders')

@section('content')
<div class="space-y-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900">My File Manager</h1>
            <p class="mt-1 text-sm text-slate-500">Create folders, filter images, and move your files.</p>
        </div>
        ...
        return `
            <article class="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                ...
                <div class="space-y-3 p-4">
                    ...
                    <button type="button" onclick="deleteCustomerImage(${img.id})"
...
async function refreshCustomerGallery() {
    ...
    try {
        ...
        renderFolders();
        renderImages();
        setGalleryStatus(`Loaded ${visibleImages().length} image(s).`, 'success');
    } catch (error) {
...
async function deleteCustomerImage(id) {
    ...
    const result = await response.json();
    if (response.ok) refreshCustomerGallery();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    refreshCustomerGallery();
    document.getElementById('refreshGalleryBtn')?.addEventListener('click', refreshCustomerGallery);
